...
import jwt
from apps.user.models import PreferOttContentGenre, User
...
from rest_framework.permissions import AllowAny, IsAuthenticated  # 토큰 있는지 확인
from rest_framework.response import Response
from rest_framework.views import APIView

from .models import ReviewScore, ReviewScoreT
from .serializer import FifthReviewScoreSerializer, ThirdReviewScoreSerializer
...
class FirstAnalysisView(APIView):  # figma의 유저설명.
    ...
    def post(self, request):
        ...
        user_prefer_genres = (
            user.prefer_ott_content_genres.all()
        )  # PreferOttContentGenre의 id(preferottcontentgenre_id)가 들어있음.
...
# 3번 ReviewScoreT
class ThirdAnalysisView(
    APIView
):  # http://127.0.0.1:8000/api/contents-analysis/third-analysis?search-gender=average_female&search-age=1999-04-27
    permission_classes = (AllowAny,)
    # permission_classes=(IsAuthenticated, )
    def post(self, request):
        ...
        if (search_gender == None) & (search_age == None):
            ...
            queryset = ReviewScoreT.objects.filter(
                Q(people=search_gender) | Q(people=search_age)
            )
            ...
            response.append(
                {
                    ...
                    "people": i["people"],
                    "data": [
                        {"genre": "드라마", "rating": i["drama"]},
                        {"genre": "코미디", "rating": i["comedy"]},
                        {"genre": "액션", "rating": i["action"]},
                        {"genre": "스릴러", "rating": i["thriller"]},
                        {"genre": "로맨스", "rating": i["romance"]},
                        {"genre": "범죄", "rating": i["crime"]},
                        {"genre": "모험", "rating": i["adventure"]},
                        {"genre": "애니메이션", "rating": i["animation"]},
                        {"genre": "판타지", "rating": i["fantasy"]},
                        {"genre": "가족", "rating": i["family"]},
                        {"genre": "SF", "rating": i["sci_fi"]},
                        {"genre": "미스터리", "rating": i["mystery"]},
                        {"genre": "공포", "rating": i["horror"]},
                        {"genre": "다큐멘터리", "rating": i["documentary"]},
                        {"genre": "전기", "rating": i["biography"]},
                        {"genre": "역사", "rating": i["history"]},
                        {"genre": "음악", "rating": i["music"]},
                        {"genre": "단편", "rating": i["short"]},
                        {"genre": "스포츠", "rating": i["sport"]},
                        {"genre": "전쟁", "rating": i["war"]},
                        {"genre": "뮤지컬", "rating": i["musical"]},
                        {"genre": "서부", "rating": i["western"]},
                    ],
                }
            )
        # return Response(target_analysis_serializer.data, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        ...
        if (search_genre1 == None) & (search_genre2 == None) & (search_genre3 == None):
            ...
            queryset = ReviewScore.objects.filter(
                Q(review_genre=search_genre1)
                | Q(review_genre=search_genre2)
                | Q(review_genre=search_genre3)
            )
            ...
            response.append(
                {
                    ...
                    "data": [
                        {"genre": "남", "rating": i["average_male"]},
                        {"genre": "여", "rating": i["average_female"]},
                        {"genre": "10s", "rating": i["average_10s"]},
                        {"genre": "20s", "rating": i["average_20s"]},
                        {"genre": "30s", "rating": i["average_30s"]},
                        {"genre": "40s", "rating": i["average_40s"]},
                        {"genre": "50이상", "rating": i["average_50_up"]},
                    ],
                }
            )
        return Response(response, status=status.HTTP_200_OK)
